...
export default function ExitIntentModal() {
  ...
  useEffect(() => {
    setMounted(true);
    // Only show for free/guest users — check quick client-side signal
    // (full premium check happens lazily; modal is low-cost to dismiss)
    ...
    function handleMouseLeave(e: MouseEvent) {
      // Trigger when cursor leaves near the top of the viewport
      if (fired || e.clientY > 40) return;
      fired = true;
      sessionStorage.setItem(SESSION_KEY, "1");

      // Don't show for premium users — check quickly
      fetch("/api/me", { cache: "no-store" })
        .then((r) => r.ok ? r.json() : null)
        .then((data) => {
          const isPremium = data?.isPremium || data?.premium;
        ...
        .catch(() => setVisible(true)); // Show on error (assume free)
    }
      ...
      <div className="w-full max-w-md overflow-hidden rounded-3xl shadow-2xl" style={{ background: 'rgba(10,8,20,0.97)', border: '1px solid rgba(168,85,247,0.4)' }}>
        ...
        <div className="px-8 py-8 text-center">
          ...
          <p className="mt-2 text-lg font-semibold text-orange-300">
            You have 2 free quizzes left this week
          </p>
